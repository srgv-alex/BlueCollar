...
task('watch', function () {
	...
	watch('./src/img/svgsprite/**/*.svg', series('sprite'));
	watch(
		['./src/img/**/*.svg', '!./src/img/svgsprite/**/*.svg'],
		series('copySvg')
	);
});

task('sprite', function () {
	return src('./src/img/svgsprite/**/*.svg')
		.pipe(
			svgSprite({
				mode: {
					symbol: {
...
task('copySvg', function () {
	return src(['./src/img/**/*.svg', '!./src/img/svgsprite/**/*.svg']).pipe(
		dest('./build/img/')
	);
});
task('svg', parallel('sprite', 'copySvg'));

task(
	'default',
	series(
		'clean',
		parallel('html', 'sass', 'js', 'images', 'fonts', 'svg'),
